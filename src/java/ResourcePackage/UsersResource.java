/*
This is the Users resource of the REST API.
It is mainly in charge of accepting API calls from clients and then using the UserDao to manipulate the user objects.

It also has an API path for the initial authentication of the user

Almost every method returns a standard HTTP response code depending on the validity of the request.
 */
...
public class UsersResource {
    ...
    @POST
    @Produces("text/plain")
    public Response registerNewUser(@FormParam("name") String name, @FormParam("password") String password, @FormParam("firstname") String firstname, @FormParam("lastname") String lastname, @FormParam("department") String department, @FormParam("title") String title, @FormParam("email") String email) {
    ...
    allusers map from UserDao class has to be converted into an ArraList by iterating through it
     */
    @GET
        ...
        if (userdao.getUsers().containsKey(userid)) {
            if (headers.getRequestHeaders().keySet().contains("authorization")) {
                ...
                if (authResult && myID == userid) {
                    ...
                    /*
                    Iterating through the message HashMap and adding all its values to an ArrayList because the 
                    @Produces annotation does not support maps
                     */
    ...
    public Response getUserGroups(@PathParam("userid") int userid, @Context HttpHeaders headers) {
        ...
        if (userdao.getUsers().containsKey(userid)) {
            if (headers.getRequestHeaders().keySet().contains("authorization")) {
                ...
                if (authResult && myID == userid) {
                    /*
                    Iterating through the group HashMap and adding all its values to an ArrayList because the 
                    @Produces annotation does not support maps
                     */
